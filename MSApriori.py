...
def main(*args):
    global must_have, cannot_be_together, c_count, tail_count
    ...
    file_read(transaction_file_name, parameter_file_name, T, MS)
    F, c_count, tail_count = MSApriori(T, MS)
    if(cannot_be_together != []):
        apply_cannot_be_together_constraint(F)
    if(must_have != []):
        apply_must_have_constraint(F)
    write_output(output_file_name, F)

def file_read(transaction_file, parameter_file, transaction_list, mis_dict):
    global sdc, must_have, cannot_be_together
    ...
    for item in M:
        for transaction in T:
            if item in transaction:
                c_count[item] = c_count.get(item, default_sup_count) + 1

    i = 0
    while i < num_of_items:
        if M[i] in c_count.keys() and c_count[M[i]] / num_of_transactions >= MS[M[i]]:
            L.append(M[i])
            break
        i = i + 1

    for j in range(i + 1, num_of_items):
        if M[j] in c_count.keys() and c_count[M[j]] / num_of_transactions >= MS[M[i]]:
            L.append(M[j])
...
def MScandidate_gen(Fk_1, T, MS, sup_count):
    global sdc, item_pos
...
def MSApriori(T, MS):
    ...
    while len(F) != 0:
        ...
        if k == 2:
            ...
            Ck = MScandidate_gen(Fk_1, T, MS, c_count)
...
def apply_cannot_be_together_constraint(F):
